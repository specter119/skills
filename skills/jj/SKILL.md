...
jj push                                  # Push all changed bookmarks to remote (uses jj-pre-push alias)
jj push --bookmark my-feature            # Push only a specific bookmark
jj push --deleted                        # Push bookmark deletions to remote (after jj abandon)
```
...
> **Note:** `jj push` is a custom alias that runs `jj-pre-push push`, which checks pre-commit hooks before pushing. If you need to bypass checks, use `jj git push` directly.

**Cleanup after abandoning a pushed change:**
When you `jj abandon` a change that had a bookmark pushed to remote, the bookmark is deleted locally. Use `jj push --deleted` to sync that deletion to the remote.
...
| Push to remote | `jj push` |
| Push specific bookmark | `jj push --bookmark <name>` |
| Push bookmark deletions | `jj push --deleted` |
